...
| `status_source` / `verification_source` / `route_source` / `retry_source` | 各値の出所。`route_source`は最終Routeの互換用出所。旧Metricsや根拠なしは`null`。 |
| `initial_route_source` / `final_route_source` | 初期Routeと最終Routeをそれぞれ判定した証拠。Rootのモデル名や子のspawnだけではRootの役割を推定せず、証拠がなければ`null`。 |
